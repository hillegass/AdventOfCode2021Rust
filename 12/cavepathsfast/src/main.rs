    ...
    for next_node in graph.get(last_node).unwrap() {
        if is_lowercase(next_node) {
            if to_avoid.contains(next_node) {
                if has_free_pass {
                    sum += number_of_paths_not_using(&next_node, to_avoid, graph, false);
                }
            } else {
                let node_name = next_node.to_string();
                to_avoid.insert(node_name);
                sum += number_of_paths_not_using(&next_node, to_avoid, graph, has_free_pass);
                to_avoid.remove(next_node);
            }
        } else {
            sum += number_of_paths_not_using(&next_node, to_avoid, graph, has_free_pass);
        }
...
fn main() {
    ...
    let path_count =
        number_of_paths_not_using(&String::from("start"), &mut HashSet::new(), &map, true);
